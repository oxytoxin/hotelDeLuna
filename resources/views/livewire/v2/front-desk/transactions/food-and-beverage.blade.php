<div x-data="{ formOpen: false }" x-on:close-form.window="formOpen=false">
  <div class="px-4 pt-4 sm:px-6 lg:px-8" x-animate>
    <div class="flex items-center justify-center">
      ...
      <div>
        <x-my.button-primary py="py-1" x-on:click="formOpen = !formOpen" label="Add New" />
      </div>
    </div>
      ...
      <div class="p-4 mt-5 bg-gray-100 rounded-lg">
        <div>
          <div class="grid grid-cols-1 gap-4">
            ...
            <x-native-select label="Select name" wire:model.debounce.500ms="form.name">

              @foreach ($foods as $food)
                <option value="{{ $food['name'] }}">{{ $food['name'] }}</option>
              @endforeach
            </x-native-select>
            <x-my.input label="Price" wire:model.debounce.500ms="form.price" numberOnly type="number" />
            <x-my.input label="Quantity" numberOnly type="number" wire:model.debounce.500ms="form.quantity" />
          </div>
          <div class="pt-2 mt-2 border-t">
            <dl class="pt-6 space-y-6 text-sm font-medium text-gray-500 border-t border-gray-200">
              ...
              <div class="flex items-center justify-between pt-6 text-gray-900 border-t border-gray-200">
                ...
                <dd class="text-base">
                  ₱
                  {{ $form->price ? $form->price * $form->quantity : '0' }}
                </dd>
              </div>
            </dl>
          </div>
