<div>
  <div class="fixed top-10 right-10">
    ...
    this.closest('form').submit();"
        label="Logout" icon="logout" slate />
    </form>
  </div>
  <div class="mb-10">
  ...
  <div>
    <div>
      <x-my.input label="Select your Name" wire:model.debounce.500ms="search" placeholder="Search" />
    </div>
    <div>
      <div class="mt-6 flow-root">
        <ul role="list" class="-my-5 divide-y divide-gray-200">
          @foreach ($frontdesks as $frontdesk)
            <li wire:key='ff{{ $frontdesk->id }}' class="py-4">
              <div class="flex items-center space-x-4">
                ...
                <div>
                  <button type="button" wire:click="select({{ $frontdesk->id }},'{{ $frontdesk->name }}')"
  ...
  <div wire:key="selected" class="mt-20">
    @if (count($selecteds) > 0)
      <div>
        ...
        <div class="buttons mt-3 flex justify-center">
          <x-button positive wire:click="startShift">
            Start Shift
          </x-button>
        </div>
      </div>
