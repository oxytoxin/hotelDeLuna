<div>
  <x-my.modal title="Select Deposits" width="sm:max-w-xl" :showOn="['show-deposits-modal']" :closeOn="['close-deposits-modal']">
    @if ($guest)
      <div class="grid gap-4">
        ...
        @if ($guest->deposit_balance < $payableAmount)
          <div class="">
            <span class="text-sm text-blue-500">Note: Deposit amount is not enough. Please enter the amount in
              addition to the total balance.</span>
            <x-my.input type="number" label="" wire:model.debounce.500ms="additionalAmount" />
          </div>
          <div class="">
    ...
    <x-slot:footer>
      <div class="flex space-x-3">
        <x-my.button-success wire:click="save" label="Pay" />
      </div>
    </x-slot:footer>
  </x-my.modal>
</div>
