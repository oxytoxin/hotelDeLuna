      ...
      <table id="example" class="table-auto mt-5" style="width:100%">
        <thead class="font-normal">
          <tr>
            ...
            <th class="border border-gray-700 text-left px-2 text-sm font-semibold text-gray-700 py-2">TIME ENTRY AS
              CLEAN</th>
            <th class="border border-gray-700 text-left px-2 text-sm font-semibold text-gray-700 py-2">ROOMBOY NAME</th>
            <th class="border border-gray-700 text-left px-2 text-sm font-semibold text-gray-700 py-2">CLEANING TIME
              INTERVAL</th>
          </tr>
        </thead>
        <tbody class="">
          ...
          @foreach ($cleanings as $item)
            <tr>
              ...
              @endphp
              <td class="border border-gray-700 px-3 py-1">{{ $guest->guest->name }}</td>
              <td class="border border-gray-700 px-3 py-1">
                {{ \Carbon\Carbon::parse($guest->guest->check_in_at)->format('m/d/Y') }} |
                {{ \Carbon\Carbon::parse($guest->guest->check_in_at)->format('g:i A') }}</td>
              <td class="border border-gray-700 px-3 py-1">
                {{ \Carbon\Carbon::parse($guest->guest->check_out_at)->format('m/d/Y') }} |
                {{ \Carbon\Carbon::parse($guest->guest->check_out_at)->format('g:i A') }}</td>
              <td class="border border-gray-700 px-3 py-1">
                {{ \Carbon\Carbon::parse($item->started_at)->format('g:i A') }}</td>
              <td class="border border-gray-700 px-3 py-1">{{ $item->room_boy->user->name }}</td>
              <td class="border border-gray-700 px-3 py-1">{{ $time_interval }}</td>
            </tr>
          @endforeach
