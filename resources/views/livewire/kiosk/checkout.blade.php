        ...
        <div class="flex-1 relative">
                        ...
                        <div class="flex justify-between">
                            <dt class="">
                                <div class="flex flex-col">
                                    <div class="uppercase text-white border-b font-bold">{{$transaction->check_in_detail->rate->type->name}}</div>
            ...
            @endforelse
              ...
              <div class="flex pt-7 px-10 justify-between">
                ...
                <dd class="text-lg font-bold text-white">&#8369;{{number_format($transactions->where('transaction_type_id',2)->sum('payable_amount'),2)}}</dd>
            </div>
            <div class="absolute border-b bottom-5 w-full left-0">
                <div class="flex w-full  px-10 items-center justify-between">
                    ...
                    <dd class="text-2xl font-bold text-white">&#8369;{{number_format($transactions->sum('payable_amount'),2)}}</dd>
                </div>
            </div>
            ...
            <div class="flex-none p-6 text-center">
              ...
              <div class="flex flex-auto flex-col justify-between p-6">
                <dl class="grid grid-cols-1 gap-x-6 gap-y-3 text-sm text-white">
                  ...
                  <dd class="truncate"><a href="https://example.com" class="text-white ">{{$guests['qr_code'] ?? '***********'}}</a></dd>
...
<div x-show="scan" x-cloak class="relative z-10" role="dialog" aria-modal="true">
    ...
    <div class="fixed inset-0 z-10 overflow-y-auto p-4 sm:p-6 md:p-20">
      ...
      <div class="relative  h-full">
        <img src="{{asset('images/scanning.gif')}}" class="absolute h-full object-cover" alt="">
        <div class="relative flex h-full items-end justify-center">
            <span class=" mb-10 bg-gray-300 rounded-full px-3 shadow-sm animate-pulse text-red-500">Please Scan your Qr Transaction code</span>
        </div>
       <input wire:model="scanner" type="text" class="sr-only" autofocus >
      
    </div>
      </div>
      <div class="grid place-content-center mt-2">
        <x-button href="{{route('kiosk.transaction')}}" md icon="arrow-circle-left" label="BACK" white  class="font-semibold"/>    
    </div> 
