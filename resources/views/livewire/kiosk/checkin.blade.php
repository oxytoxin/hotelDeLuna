        ...
        @case(2)
          <div class="mx-10 mt-5 font-rubik">
            ...
            <div class="div">
              <div class="my-5 space-x-3  flex">
                @foreach ($floors as $key => $floor)
                  @if ($floor->rooms->where('room_status_id', 1)->where('priority', 1)->where('type_id', $type_key)->count() > 0)
                    <button wire:click="$set('floor_id', {{ $floor->id }})"
                      class="{{ $floor_id == $floor->id ? 'bg-green-500 text-white border-white' : '' }} bg-white border-4 border-green-500 text-gray-700 hover:bg-green-500 hover:border-white hover:text-white p-2 px-4 shadow-lg rounded-full">
                      <span class="text-2xl font-bold  uppercase">{{ ordinal($floor->number) }}
                        FLOOR</span>
                    ...
                    <div class="bg-white  relative xl:h-64 lg:h-60 rounded-3xl ">
                      ...
                      <div class="relative flex flex-col justify-center items-center h-full">
                        <svg class="h-16 text-gray-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48"
                          fill="none">
                          <path
                            d="M22.5033 7.77637L37.0033 10.7764C37.5837 10.8965 38 11.4077 38 12.0004V36.0005C38 36.5932 37.5837 37.1044 37.0033 37.2245L22.5033 40.2245C21.7275 40.385 21 39.7926 21 39.0005V9.00045C21 8.20826 21.7275 7.61587 22.5033 7.77637ZM18 10.0004L18.1278 37.994L18 38.0005H11.25C10.6028 38.0005 10.0705 37.5086 10.0065 36.8783L10 36.7505V11.2504C10 10.6032 10.4919 10.0709 11.1222 10.0069L11.25 10.0004H18ZM26.5 22.5005C25.6716 22.5005 25 23.172 25 24.0005C25 24.8289 25.6716 25.5005 26.5 25.5005C27.3284 25.5005 28 24.8289 28 24.0005C28 23.172 27.3284 22.5005 26.5 22.5005Z"
                            fill="currentColor"></path>
                        </svg>
                        <h1 class="font-black text-4xl text-center text-gray-600">ROOM
                          #{{ $room->number }}
