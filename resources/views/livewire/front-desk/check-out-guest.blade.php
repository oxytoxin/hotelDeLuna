<div x-data x-init="$refs.search.focus()">
  <div class="flex p-2 space-x-3 bg-white border border-gray-300 rounded-lg">
    <div class="w-1/2">
      <x-input placeholder="Search Guest" wire:model.defer="search" x-ref="search" />
    </div>
    <x-button label="Qr Code" icon="search" wire:click="search('qr_code')" spinner="search('qr_code')" primary />
    <x-button label="Room Number" icon="search" wire:click="search('phone_number')" spinner="search('phone_number')"
      primary />
    @if ($this->guest)
      <x-button negative label="Clear" wire:click="clear" spinner="clear" />
    @endif
  </div>
  <div wire:key="main" class="mt-5" x-animate>
    @if ($this->guest)
                    ...
                    <x-transactions.cell>
                      <div class="flex space-x-3">
                        @if ($deposit->claimed_at)
                          Claimed
                        @else
                          <x-button positive wire:click="claimDeposit({{ $deposit->id }})" xs>
                            Claim
                          </x-button>
                        @endif
                      </div>
                            ...
                            <tr wire:key="rows{{ $transaction_type_id . $key }}" class="border-t border-gray-300">
                              ...
                              <td class="px-3 py-4 text-sm text-gray-500 whitespace-nowrap">
                                @if ($transaction->paid_at)
                                  {{ $transaction->paid_at }}
                                @else
                                  <x-button positive label="Pay" xs
        ...
        <div wire:key="button-checkOut">
          <div>
            @if ($this->guest->totaly_checked_out == false)
              <x-button label="Check Out" lg wire:click="checkOut" class="w-full" ner="checkOut" positive />
            @endif
          </div>
        </div>
      </div>
  ...
  <div wire:key="modal">
    <x-modal.card title="Override Action" max-width="sm" wire:model.defer="override.modal">
      <form class="grid gap-3">
        @csrf
        <x-input label="Amount" wire:model.defer="override.new_amount" type="number" />
        <x-input label="Authorization Code" wire:model.defer="override.authorization_code" type="password" />
      </form>
      <x-slot:footer>
        <x-button primary wire:click="overrideTransaction">
          Override
        </x-button>
      </x-slot:footer>
    </x-modal.card>
