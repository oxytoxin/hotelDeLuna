<div class="mx-auto max-w-7xl sm:px-6 lg:px-8" x-data="{ addpanel: false }">
  <div class="header flex items-center justify-between">
    <div class="welcome">
      <h1 class="text-2xl font-semibold uppercase text-gray-600">{{ $category->name }}</h1>
    ...
    <div class="search flex space-x-2">
      <div class="flex bg-gray-50 p-1 px-3 space-x-1 items-center rounded-lg">
        ...
        <input wire:model="searchMenu" type="text"
          class="w-56 border-0 focus:border-0 bg-transparent focus:ring-0 text-sm" placeholder="Search for Menu....">
      </div>
      {{-- <button x-on:click="addpanel = true" class="bg-gray-600 text-white px-3 hover:bg-gray-700 rounded-lg">
        <span>Add New</span>
      </button> --}}
      <x-button wire:click="$set('add_modal', 2)" label="Add New" right-icon="plus" dark />
    </div>
  </div>
      ...
      <div class="bg-gray-50 rounded-xl shadow p-2 h-[15.5 rem]">
        <div class="bg-gray-500 shadow h-40 rounded-xl relative">
          ...
          <div class="absolute top-2 right-2">
            <div wire:click="editMenu({{ $menu->id }})"
  ...
  {{-- <div x-show="addpanel" x-cloak class="relative z-10" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <!--
      ...
      Leaving: "ease-in duration-200"
        ...
        To: "opacity-0"
    -->
    <div x-show="addpanel" x-cloak x-transition:enter="transition ease-out duration-300"
          ...
          <div>
            ...
            <div class="relative mt-5">
              <div
                ...
                <label for="description" class="sr-only">Description</label>
                <input type="number" wire:model="price" name="" id=""
                  class="w-full focus:ring-0 placeholder-gray-500 sm:text-sm border-0 py-0 appearance-none mb-2"
                  placeholder="&#8369;100.00">
    ...
    <x-card>
      <div>
        ...
        <div class="relative mt-5">
          <div
            ...
            @enderror border-gray-300 rounded-lg shadow-sm overflow-hidden focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500">
            <label for="title" class="sr-only">Category Name</label>
            <h1 class="p-2 font-medium placeholder-gray-500"> Food Name</h1>
            <label for="description" class="sr-only">Description</label>
            <textarea wire:model="name" rows="2"
      ...
      <x-slot name="footer">
        <div class="flex justify-end gap-x-2">
          ...
          <x-button wire:click="saveMenu" spinner="saveMenu" label="Save Menu" positive />
        </div>
      </x-slot>
    </x-card>
  </x-modal>
  ...
  <x-modal align="center" max-width="md" wire:model.defer="edit_modal">
    <x-card>
      <div>
        <h1 class="font-semibold text-gray-600">Edit Menu</h1>
        <div class="relative mt-5">
          <div
            class="border @error('name')
            text-red-500 border-red-500
      ...
      <x-slot name="footer">
        <div class="flex justify-end gap-x-2">
          <x-button x-on:click="close" label="Close" flat negative />
          <x-button wire:click="updateMenu" spinner="updateMenu" label="Update Menu" positive />
        </div>
      </x-slot>
    </x-card>
  </x-modal>
