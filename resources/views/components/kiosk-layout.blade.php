...
<body class="font-sans antialiased h-full bg-gray-700" x-data="{ logout: false }">
    ...
    <div x-show="logout" x-cloak class="relative z-10" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        ...
        x-transacion:enter="ease-out duration-300"
        x-transacion:enter-start="opacity-0"
        x-transacion:enter-end="opacity-100"
        x-transacion:leave="ease-in duration-200"
        x-transacion:leave-start="opacity-100"
        x-transacion:leave-end="opacity-0"
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto">
          <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            this.closest('form').submit();"
                ...
                <button x-on:click="logout=false" class="mt-3 inline-flex w-full justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-base font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:mt-0 sm:w-auto sm:text-sm">Cancel</button>
              </div>
            </div>
          </div>
        </div>
